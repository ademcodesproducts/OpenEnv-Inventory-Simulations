"""
Qwen2.5-Instruct inference agent for inventory optimization.

Loads a Qwen2.5-Instruct base model (optionally with a LoRA adapter) and drives
inventory episodes with the same external interface as ClaudeInventoryAgent.

Usage:
    python -m agent.finetune_agent --env-type 0 --base-url http://localhost:7860
    python agent/finetune_agent.py --model Qwen/Qwen2.5-3B-Instruct --adapter-path ./lora_ckpt
"""
...
class QwenInventoryAgent:
    """Inventory optimization agent backed by a local Qwen2.5-Instruct model."""
    ...
    def decide(self, obs: InventoryObservation) -> tuple[float, str, float]:
        ...
        Falls back to a safety-stock formula if the model output cannot be parsed.
        """
        messages = [
        ...
        try:
            ...
            raw_text: str = self._tokenizer.decode(new_tokens, skip_special_tokens=True)
            ...
            reasoning = f"Inference error — fallback used: {exc}"
            confidence = 0.0
    ...
    def reset(self) -> None:
        """Clear per-episode state (memory bank)."""
        self._memory_bank = []
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a QwenInventoryAgent episode against the inventory server."
    )
    parser.add_argument(
        "--model",
        type=str,
        default=QwenInventoryAgent.MODEL,
        help="Base model name (HuggingFace hub) or local path to a Qwen2.5-Instruct checkpoint.",
    )
    parser.add_argument(
        "--adapter-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help="Device map for model loading (e.g. 'auto', 'cpu', 'cuda:0').",
    )
